...
(() => {
  ...
  // ---- 중앙 상태 저장소 ----
  // ⚠️ 단어장 전용 버전(Step4): 문장/회화/쉐도잉 관련 상태는 제거
  App.State = {
    // 단어 데이터
    vocabulary: [],
    currentFileName: '',
    ...
    totalQuizHistory: (function(){
      try { return JSON.parse(localStorage.getItem('totalQuiz')||'[]') || []; } catch(e){ return []; }
    })(),
  ...
  // (호환용 스텁) 일부 레거시 코드가 dialogScript/dialogFileName을 참조할 수 있어 빈 값으로 제공
  // ※ Step5~에서 parser/render를 제거하면 이 스텁도 제거 예정
  let _dialogScriptStub = [];
  let _dialogFileNameStub = '';
  def('dialogScript', () => _dialogScriptStub, (v) => { if (Array.isArray(v)) _dialogScriptStub = v; });
  def('dialogFileName', () => _dialogFileNameStub, (v) => { _dialogFileNameStub = (v ?? ''); });
